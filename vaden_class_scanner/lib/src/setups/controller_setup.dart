...
(Handler inner) {
  ...
  for (final method in _getAllMethods(classElement)) {
    ...
    final apiPathResolver = '$controllerPath$routePath'.replaceAll('<', '{').replaceAll('>', '}').replaceAll('//', '/');
...
""");
        ...
        paramCodeList.add("final ${parameter.name} = request.context['$cname'] as dynamic;");
      } else {
